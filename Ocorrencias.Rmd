...
# Documentação de todas as fontes de ocorrência do Katsuwonus Pelamis (Bonito-Listrado ou Skipjack Tuna)
...
leaflet() %>%
  ...
  addRectangles(
    ...
    color = "blue", weight = 2,  # Cor e espessura da borda
    fill = FALSE, popup = "Área Oceano Atlântico Sul"
  )
```
...
obis_data <- read.csv("~/RFiles/ocorrencias_katsuwonus/obis/Occurrence.csv", header = TRUE, sep = ",", quote = "\"", stringsAsFactors = FALSE)
names(obis_data)
...
nrow(obis_data_filtered_ext) # 105025 - mesmo número de linhas que os dados normais, ou seja, todos os dados estão nesses intervalos
```
...
table(obis_data_filtered_sul$year)
# fazer mais alguma coisa? ano minimo e maximo, histogramas, ...
```
...
```{r}
# Carregando o arquivo
gbif_data <- read.csv("~/RFiles/ocorrencias_katsuwonus/gbif/0030473-241007104925546.csv", sep='\t')
names(gbif_data)
...
As coordenadas de latitude e longitude são definidas como valores
contínuos, com 4 casas decimais. Os valores que possuem mais casas são
apenas arredondamentos, a princípio
...
nrow(gbif_data_filtered_ext) # 28279 - mesmo número de linhas que os dados normais, ou seja, todos os dados estão nesses intervalos
```
...
table(gbif_data_filtered_sul$year)
# fazer mais alguma coisa? ano minimo e maximo, histogramas, ...
```
...
![<https://www.iccat.int/Data/ICCAT_maps.pdf>](~/RFiles/outros/regioes.png)
...
```{r}
# (acho que task 1 não é preciso)
# Para os dados referentes à Task 1, foi usado dataset ... . Para baixar,
# basta ...
# ### Exploração inicial (Task 1)
```
...
```{r}
# Carregando o arquivo
iccat_data <- read.csv("~/RFiles/ocorrencias_katsuwonus/iccat/t2sz_SKJ1950-22/t2sz_SKJ1950-22.csv")
names(iccat_data)
...
iccat_data_filtered <- iccat_data %>%
  drop_na(Lon, Lat)
nrow(iccat_data_filtered)
```

```{r}
# Todos os valores de lat/lon são absolutos
...
Ver página 13 do "ICCAT MAPS"
![https://www.iccat.int/Data/ICCAT_maps.pdf](~/RFiles/outros/quadrantes.png)

```{r}
# número de peixes (maioria tem valores bem baixos)
hist(iccat_data_filtered$Nr)
```
...
identificar a área conforme o código da região
...
nrow(iccat_data_filtered_1x1_ext) # 717901 - mesmo número de linhas que os dados normais, ou seja, todos os dados estão nesses intervalos
```
...
table(iccat_data_filtered_1x1_sul$YearC)
# fazer mais alguma coisa? ano minimo e maximo, histogramas, ...
```
