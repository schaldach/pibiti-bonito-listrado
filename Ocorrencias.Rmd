...
```{r}
# importante usar a opção Knit -> Knit Directory -> Current working directory para mudar o diretório de trabalho
# para a pasta com os arquivos pelo RStudio
...
knitr::include_graphics("imagens/regioes.png", error = FALSE)
```
...
```{r}
# https://www.iccat.int/Data/ICCAT_maps.pdf
knitr::include_graphics("imagens/quadrantes.png", error = FALSE)
```

```{r}
# número de peixes (maioria tem valores bem baixos)
hist(iccat_data_filtered$Nr, main="Número de peixes")
```
